...
std::string mkdir::run(const std::vector<std::string>& args)
{
    if (args.size() < 3)
...
std::string ls::run(const std::vector<std::string>& args)
{
...
std::string touch::run(const std::vector<std::string>& args)
{
    if (args.size() < 3)
    {
    ...
    {
        std::cout << "Fisierul deja exista.\n";
    }
    else
    {
        std::cout << "Creare fisier: " << file_path << "\n";
        std::ofstream(file_path, std::ios::trunc);
    }
    return args[0];
}
touch::~touch() {}

std::string rm::run(const std::vector<std::string>& args)
{
    if (args.size() < 3)
    {
...
std::string mv::run(const std::vector<std::string>& args)
{
    if (args.size() < 3)
    {
...
std::string cd::run(const std::vector<std::string>& args)
{
    if (args.size() < 3)
    {
